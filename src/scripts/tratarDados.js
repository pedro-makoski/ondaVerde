...
botao_positions.addEventListener("click", () => {
    if(passed === 1) {
        ...
        times = mesclarInString(times, valores.positions, ' segundo(s): ', ' metro(s)', (timeList, positionlist, idx, resList) => {
            if(timeList[idx] < 0) {
        ...
        const timesDiferences = new ElementosInputs('<p class="diferenca-tempo">{value} segundos</p></div>')
